...
$configSQL = "SELECT `id`,`table_name`,`database_field_name`,`report_field_name`,`static_value`,`custom_function`,`merchant_id`,`description`,`order`,CASE WHEN `required` = true THEN 'Y' WHEN `required` = false THEN 'N' END AS `required`, CASE WHEN `enabled` = true THEN 'Y' WHEN `enabled` = false THEN 'N' END AS `enabled`, CASE	WHEN `editable` = true THEN 'Y'	WHEN `editable` = false THEN 'N' END AS `editable`, CASE WHEN `static` = true THEN 'Y' WHEN `static` = false THEN 'N' END AS `static` FROM `" . $GLOBALS["schema"] . "`.`merchant_center_select_config` WHERE `merchant_id`='" . $GLOBALS["merchant"] . "' ORDER BY `order`";

$configQUERY = mysql_query($configSQL);
...
function feedConfigRequired($value){
	if($value === 'Y'){return " class=\"required\"";
	} else {return "class=\"optional\"";
		}
}
...
function displayConfigFields(){
	
	while($cRow = mysql_fetch_array($GLOBALS["configQUERY"])){		
		echo "<h3 id=\"" . $cRow["report_field_name"] . "\" class=\"" . feedConfigEditableHeader($cRow["editable"]) . "\" >[" . $cRow["report_field_name"] . "]" .  feedConfigActive($cRow["enabled"]) .feedConfigRequiredCaption($cRow["required"]) . "</h3>
...
<table cellspacing=\"0\" cellpadding=\"0\" id=\"fieldSettings\">
  ...
  <tr>
    ...
    <td></td>
    <td colspan=\"2\" rowspan=\"11\" valign=\"top\"><textarea rows=\"15\" cols=\"22\" name=\"description\" wrap=\"virtual\" " . feedConfigEditable($cRow["editable"]) . ">" . $cRow["description"] . "</textarea></td>
  </tr>
  <tr>
    <td colspan=\"5\" height=\"10\"></td>
  </tr>
  ...
  <tr>
    <td colspan=\"5\" rowspan=\"4\" class=\"border\">
		<a href=\"/merchant_manager/feed_config/config_finfo.php?f=" . $GLOBALS["merch"] . "&fd=" . $cRow["id"] . "\" target=\"_blank\" title=\"View report information for this field\">View More Detailed Field Information</a>
	</td>
    <td></td>
  </tr>
  <tr>
